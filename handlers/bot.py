...
async def mentionalladmin(event):
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id,filter=ChannelParticipantsAdmins):
      ...
      usrtxt += f"\n➢ [{usr.first_name}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("** Etiketleme işlemi bitti🤗 **")
        return
      if usrnum == 5:
        await client.send_message(event.chat_id, f"{msg}\n\n{usrtxt}")
        await asyncio.sleep(3)
  ...
  if mode == "text_on_reply":
    ...
    sender = await event.get_sender()
    rxyzdev_initT = f"[{sender.first_name}](tg://user?id={sender.id})"
    if event.chat_id in rxyzdev_tagTot:await event.respond(f"** ✅ Etiketleme işlemi tamamlandı ! **.\n\n👥 ** Etiketlenen kullanıcısayısı: ** {rxyzdev_tagTot[event.chat_id]}\n🗣 ** Etiketleme işlemini başlatan: ** {rxyzdev_initT}")
